...
#define NVS_CACHE_ENABLED    // TimeSeries cache
...
#define EXTENDED_FILE_DEBUG_ENABLED // Modbus logs to  "/data/log.txt"

#ifdef BATTERY_ENABLED
#define CH_PROFILE_DOWN 0
#define CH_PROFILE_UP 1
// battery control profiles, ids 100-119 reserved for charging profiles, 121-140 for discharge
...
#define CH_PROFILE_BATT_CHARGE_EXCESS 142    // Charge 0-100%
#define CH_PROFILE_BATT_DISCHARGE_EXCESS 143 // Discharge 0-100%

#endif

// experimental remote connection
...
#define EEPROM_CHECK_VALUE 10114 //!< increment this is data structure changes
#define eepromaddr 0
...
#define VARIABLE_PVFORECAST_SUM24 20 //!< Solar power forecast for local panels, next 24 hours
...
#define SERIAL_COMMAND_STATE_WAITWIFICODE 0
...
typedef struct
{
  ...
  char forecast_loc[10];
  uint8_t solar_forecast_source; // 0 none, 1 fmi, 2 forecast.solar
  float latitude, longitude;
  int16_t declination, azimuth;
...
public:
  ...
  int32_t sum();
  void debug_print(time_t start_ts, time_t end_ts_incl, bool rows);
  void debug_print(bool rows);
  void set_store_start(time_t new_start);
  // new experimental version of time series ranking
  int get_period_rank(const int id, time_t period_ts, time_t start_ts, time_t end_ts_incl, bool);
  int get_period_rank_hour(const int id, time_t period_ts, time_t start_ts, time_t end_ts_incl, bool);
  void apply_pricemodifier(bool debug);
...
void log_msg(uint8_t type, const char *msg, bool write_to_file, bool update_last_msg);
 ...
 * @param write_to_file
 */
void log_msg(uint8_t type, const char *msg, bool write_to_file = false, bool update_last_msg = true)
{
...
bool timeSeries::save_to_cache(time_t expires) // save to nvs,true if successfull
{
  ...
  nvs_close(my_handle);
  Serial.printf("save_to_cache, wrote time series %d, %d - %d,  expiration %lu, size %lu\n", (int)id_, start(), end(), expires, sizeof(store));
...
bool timeSeries::read_from_cache(time_t expire_not_before) //  https://github.com/espressif/esp-idf/blob/b4268c874a4cf8fcf7c0c4153cffb76ad2ddda4e/examples/storage/nvs_rw_blob/main/nvs_blob_example_main.c
{
  ...
  {
    printf("Not matching store data!\n");
    return false;
  }
  else
  {
...
{
  yield();
  Serial.printf("stats ts %ld, start_ts %ld, end_ts_incl %ld, 15-mins %ld \n", ts, start_ts, end_ts_incl, (get_netting_period_start_time(end_ts_incl) - get_netting_period_start_time(start_ts)) / 900 + 1);
...
void timeSeries::debug_print(bool rows = true)
{
  debug_print(store.start, store.start + (store.n - 1) * store.resolution_sec, rows);
}
...
int timeSeries::get_period_rank(const int id, time_t period_ts, time_t start_ts, time_t end_ts_incl, bool descending = false)
{
  yield();
  int rank = 1;
  ...
  Serial.printf("get_period_rank (id:%d) period_ts %ld, start_idx %d, this_period_idx %d, end_idx %d,start_ts %d,  end_ts_incl %d, 15-mins %ld\n", id, period_ts, start_idx, this_period_idx, end_idx, start_ts, end_ts_incl, (get_netting_period_start_time(end_ts_incl) - get_netting_period_start_time(start_ts)) / 900 + 1);
...
int timeSeries::get_period_rank_hour(const int id, time_t period_ts, time_t start_ts, time_t end_ts_incl, bool descending = false)
{
  ...
  Serial.printf("get_period_rank_hour (id:%d) period_hour_ts %d, start_hour_ts %d, end_hour_ts_incl %d, hours_in_range %d\n", id, period_hour_ts, start_hour_ts, end_hour_ts_incl, hours_in_range);
...
bool update_prices_to_influx()
{
  ...
  for (time_t current_period_start_ts = prices->start(); current_period_start_ts <= prices->end(); current_period_start_ts += prices->resolution_sec())
  {
    yield();
    ...
    current_price = (long)prices->get(current_period_start_ts);
...
void readFromEEPROM()
{
  ...
  if (s.netting_period_sec == 0)
  { // TODO: should not happen normally, only if not well resetted
...
bool scan_sensors()
{
  ...
  {
    snprintf(error_msg_buf, sizeof(error_msg_buf), "Found %d sensors", sensor_count);
    log_msg(MSG_TYPE_INFO, error_msg_buf, true);
  }
...
{ // Callback to monitor errors
  ...
  if (event != Modbus::EX_SUCCESS)
  {
    ...
    sprintf(error_msg_buf, PSTR("Modbus error: 0x%03X in (trans %u)"), event, transactionId);
...
void calculate_forecast_variables()
{
  ...
  {
    for (time_t period = current_hour_start_ts; period < current_period_start_ts + SECONDS_IN_DAY; period += SOLAR_FORECAST_RESOLUTION_SEC)
    {
      price = prices->get(period, VARIABLE_LONG_UNKNOWN);
...
void calculate_price_rank_variables()
{
  ...
  Serial.printf("calculate_price_rank_variables start: %ld, end: %ld, current_period_start_ts: %lu\n", prices_record_start, prices->end() + PRICE_RESOLUTION_SEC, current_period_start_ts);
  if (prices->get(time(nullptr), VARIABLE_LONG_UNKNOWN) == VARIABLE_LONG_UNKNOWN)
  {
    if (use_prices)
    {
  ...
  long price_now_raw = (long)prices->get(time(nullptr));
  ...
  // 9 h sliding
  // time_t last_ts_in_window = min(current_period_start_ts + 8 * prices->resolution_sec(), prices->last_set_period_ts());
  // Serial.printf("\n current_period_start_ts %ld last_ts_in_window %ld, A %ld,  B %ld\n", current_period_start_ts, last_ts_in_window, current_period_start_ts + 8 * prices->resolution_sec(), prices->last_set_period_ts());

  // rank = prices->get_period_rank(current_period_start_ts, last_ts_in_window - 8 * prices->resolution_sec(), last_ts_in_window);
  ...
  // MTU15M
  last_ts_in_window = min(prices->period_start(current_period_start_ts + 9 * SECONDS_IN_HOUR - 1), prices->last_set_period_ts());
  first_ts_in_window = last_ts_in_window - 9 * SECONDS_IN_HOUR + prices->resolution_sec();
  rank = prices->get_period_rank(VARIABLE_PRICERANK_15_9, current_period_start_ts, first_ts_in_window, last_ts_in_window);
  vars.set(VARIABLE_PRICERANK_15_9, (long)rank);

  // prices->stats(current_period_start_ts, last_ts_in_window - 8 * SECONDS_IN_HOUR, last_ts_in_window, &window_price_avg, &price_differs_avg, &price_ratio_avg);
  ...
  // 24 h sliding
  time_t last_hour_ts_in_window = min(current_hour_start_ts + 23 * SECONDS_IN_HOUR, prices->last_hour_ts());
  last_ts_in_window = min(prices->period_start(current_period_start_ts + 24 * SECONDS_IN_HOUR - 1), prices->last_set_period_ts());
  first_ts_in_window = last_ts_in_window - 24 * SECONDS_IN_HOUR + prices->resolution_sec();
  // rank = prices->get_period_rank(current_period_start_ts, last_ts_in_window - 23 * prices->resolution_sec(), last_ts_in_window);
  rank = prices->get_period_rank_hour(VARIABLE_PRICERANK_24, current_period_start_ts, last_hour_ts_in_window - 23 * SECONDS_IN_HOUR, last_hour_ts_in_window);
  ...
  // MTU15M
  // käykö edellinen last_ts_in_window = min(current_period_start_ts + 24 * SECONDS_IN_HOUR-1, prices->last_set_period_ts());
  rank = prices->get_period_rank(VARIABLE_PRICERANK_15_24, current_period_start_ts, last_ts_in_window - 24 * SECONDS_IN_HOUR + prices->resolution_sec(), last_ts_in_window);
  ...
  // last_ts_in_window = first_ts_in_window + SECONDS_IN_HOUR * 24 - prices->resolution_sec();
  // first_ts_in_window = last_ts_in_window - 23 * SECONDS_IN_HOUR;
  rank = prices->get_period_rank(VARIABLE_PRICERANK_FIXED_15_24, current_period_start_ts, first_ts_in_window, last_ts_in_window);
  ...
  // rank = prices->get_period_rank(current_period_start_ts, first_ts_in_window, last_ts_in_window);
  first_ts_in_window = current_hour_start_ts - SECONDS_IN_HOUR * block_start_before_this_idx;
  last_ts_in_window = prices->period_start(first_ts_in_window + 8 * SECONDS_IN_HOUR - 1);

  // Serial.println("VARIABLE_PRICERANK_FIXED_8");
  rank = prices->get_period_rank_hour(4, current_period_start_ts, first_ts_in_window, last_ts_in_window);
  ...
  rank = prices->get_period_rank(54, current_period_start_ts, first_ts_in_window, last_ts_in_window);
  vars.set(VARIABLE_PRICERANK_FIXED_15_8, (long)rank);

  // period within current hour
  first_ts_in_window = current_hour_start_ts - SECONDS_IN_HOUR * block_start_before_this_idx;
  rank = prices->get_period_rank(VARIABLE_PRICERANK_15_HOUR, current_period_start_ts, current_hour_start_ts, current_hour_start_ts + 3599);
...
bool get_price_data_entsoe()
{
  ...
  while (client_https.available())
  {
    ...
    {                                               // header dates
      ...
      Serial.printf("Debug before get_price_data_entsoe %lu, %d", period_end, prices_in->n());
    ...
    {
      //  prices_in->set(period_start + (pos - 1) * PRICE_RESOLUTION_SEC, price);
      // MTU15M

      if (period_start + (pos - 1) * resolution <= future_wanted_max_ts)
      { // skip prices too far in the future
        prices_in->set(period_start + (pos - 1) * resolution, price);
      }
  ...
  if (end_reached && ((period_end_max - period_start_min) / SECONDS_IN_HOUR >= (MAX_PRICE_HOURS - 1))) // accept a few "missing rows", e.g. same price not duplicated
  {
    ...
    { // potential problem in the latest fetch, give shorter validity time
      ...
      prices_expires_ts = time(nullptr) + (4 * SECONDS_IN_HOUR);
    }
    else
    {
...
bool is_force_state_valid(int channel_idx)
{
  // Serial.printf("DEBUGA ch %d force_state_from_ts %ld, time(nullptr) %ld, force_state_until_ts %ld\n", channel_idx, s.ch[channel_idx].force_state_from_ts, time(nullptr), s.ch[channel_idx].force_state_until_ts);
...
// WiP channel stats
bool read_channel_stats_modbus(int channel_idx)
{
  ...
  {
    snprintf(error_msg_buf, ERROR_MSG_LEN, PSTR("Channel %d has undefined relay ip."), channel_idx + 1);
    log_msg(MSG_TYPE_WARN, error_msg_buf, false);
    return false;
  }
  yield();
  ...
  { // Check if connection to Modbus slave is established
    mb.task();
    Serial.println(F("Connection ok. Reading  Modbus registries."));

    int channel_variable_idx = 0; // RFU, later there could be more channels to store
    switch (s.ch[channel_idx].type)
    {
...
bool switch_http_relay(int channel_idx, bool up)
{
  ...
  if (error)
  {
    snprintf(error_msg_buf, ERROR_MSG_LEN, PSTR("Cannot connect channel %d switch at %s "), channel_idx + 1, s.ch[channel_idx].relay_ip.toString().c_str());
    log_msg(MSG_TYPE_WARN, error_msg_buf, false);
    Serial.println(error.f_str());
    return false;
  }
  else
  {
    ...
    if (doc.containsKey(response_key))
    {
      ...
      {
        snprintf(error_msg_buf, ERROR_MSG_LEN, PSTR("Switch for channel %d switch at %s not timely set."), channel_idx + 1, s.ch[channel_idx].relay_ip.toString().c_str());
        log_msg(MSG_TYPE_WARN, error_msg_buf, false);
      }
    }
    else
    {
      snprintf(error_msg_buf, ERROR_MSG_LEN, PSTR("Switch for channel  %d switch at %s, invalid response."), channel_idx + 1, s.ch[channel_idx].relay_ip.toString().c_str());
      log_msg(MSG_TYPE_WARN, error_msg_buf, false);
    }
    return true;
  }
...
// handle timeseries pointed by prices_in
bool get_price_data_elering(char *country_code)
{
  ...
  while (client_https.available())
  {
    ...
    if (sep1 > -1 && sep2 > -1 && sep2 > sep1)
    {
      ...
      if (ts >= history_wanted_min_ts && ts <= future_wanted_max_ts) // handle only values in range, there should be no extra from Elering anyway
      {
        ...
        if (prices_in->start() != ts_min_stored)
        {
  ...
  prices_in->apply_pricemodifier(false);
  prices_in->debug_print();
...
void create_settings_doc(DynamicJsonDocument &doc, bool include_password)
{
  ...
  doc["solar_forecast_source"] = s.solar_forecast_source;
  doc["latitude"] = s.latitude;
  doc["longitude"] = s.longitude;
  doc["declination"] = s.declination;
  doc["azimuth"] = s.azimuth;
...
bool store_settings_from_json_doc_dyn(DynamicJsonDocument doc)
{
  ...
  s.longitude = ajson_float_get(doc, (char *)"longitude", s.longitude);
  s.declination = ajson_int_get(doc, (char *)"declination", s.declination);
  s.azimuth = ajson_int_get(doc, (char *)"azimuth", s.azimuth);
...
void onWebPricesGet(AsyncWebServerRequest *request)
{
  ...
  Serial.printf("DEBUG onWebPricesGet %lu - %lu (%d)\n", prices->start(), prices->end(), prices->n());

  if (prices->start() > 0)
  {
    for (int i = 0; i < MAX_PRICE_PERIODS; i++)
    {
      prices_a[i] = prices->get(prices->start() + prices->resolution_sec() * i);
      //   Serial.print(prices->get(prices->start() + prices->resolution_sec() * i));
    }
  }

  // new time series, mieti miten nämä korvataan, myös UI
  doc["record_start"] = prices->start();                         // prices_record_start;
  doc["record_end_excl"] = prices->end() + PRICE_RESOLUTION_SEC; // prices_record_end_excl;
...
bool connect_wifi()
{
  ...
  if (WiFi.softAP(APSSID.c_str(), (const char *)__null, wifi_channel, 0, 3) == true)
  {
    ...
    delay(100);
    serial_command_state = SERIAL_COMMAND_STATE_WAITWIFICODE; // ready to receive commands from serial
  }
  else
  {
  ...
  Serial.println(F("DEBUG connect_wifi ended false"));
  return false;
}
...
// New version adopted 14.5.2025
  ...
  */
  ...
  server_web.serveStatic("/data/log.txt", FILESYSTEM, "/data/log.txt").setCacheControl("max-age=10");
...
void loop()
{
  ...
  if (todo_in_loop_scan_wifis)
  {
    ...
    scan_and_store_wifis(false, true);
  }
...
// started from admin UI
...
#ifdef BATTERY_ENABLED
    next_read_channels_stats_ts = time(nullptr);
    Serial.print("next_read_channels_stats_ts:");
    Serial.println(next_read_channels_stats_ts);
#endif
  ...
  if ((next_query_price_data_ts <= time(nullptr)) && (prices_expires_ts <= time(nullptr)) && wifi_sta_connected)
  {
    ...
    if (got_price_ok)
    {
      // When new price info succesfully received, swap pointers prices_in <-> prices
      swap_price_timeseries();

#ifdef NVS_CACHE_ENABLED
      if (!got_data_from_cache && got_price_ok)
      {
        prices->save_to_cache(prices_expires_ts);
      }
#endif
  ...
  {
    io_tasks(STATE_PROCESSING);
    if ((s.solar_forecast_source == SOLAR_FORECAST_TYPE_FMI) && (strlen(s.forecast_loc) > 1))
    {
